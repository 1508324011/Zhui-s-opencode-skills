---
name: web-access
description: 处理需要联网检索、网页读取、登录态网页操作、动态渲染页面访问与浏览器 CDP 交互的任务。适用于搜索信息、查看网页内容、访问需要登录的网站、操作网页界面、抓取社交媒体内容，以及任何需要真实浏览器环境的网络任务。
license: MIT
metadata:
    skill-author: 一泽Eze (adapted for local OpenCode installation)
...
本地 OpenCode 适配说明：本技能改编自上游 `eze-is/web-access`，并作为原生 OpenCode skill 安装到本地。OpenCode 不提供 Claude Code 的 `$CLAUDE_SKILL_DIR` 环境变量；在 OpenCode 中执行下文 shell 命令时，请**显式把 Bash 的 `workdir` 设为技能根目录** `~/.config/opencode/skills/web-access/`，或把 `./scripts/...` 与 `references/...` 替换为实际绝对路径，不要假设宿主会自动切换到本目录。运行前提保持与上游一致：**Node.js 22+ 是默认支持路径**，并且需要在 Chrome 的 `chrome://inspect/#remote-debugging` 中启用当前浏览器实例的 remote debugging。
...
- **Node.js 22+**：推荐并视为默认支持路径（脚本默认依赖原生 WebSocket）。版本低于 22 只有在**当前运行环境本身已经可解析 `ws` 模块**时才可能可用，本技能不会为你自动安装该依赖。
...
仅当当前宿主环境支持子 Agent / 并行 agent 时适用本节；如果当前环境不提供这类能力，请忽略本节并按单 Agent 流程执行。
...
已有经验的站点会以 `references/site-patterns/{domain}.md` 形式存放。如需查看当前已经记录了哪些站点，请在 shell 中把 `workdir` 设为 `~/.config/opencode/skills/web-access/` 后运行：

```bash
node -e "const fs=require('fs');const path=require('path');const p=path.resolve(process.cwd(),'references','site-patterns');try{console.log(fs.readdirSync(p).filter(f=>f.endsWith('.md')).map(f=>f.replace(/\\.md$/,'')).join(', ')||'暂无')}catch{console.log('暂无')}"
```
