...
> OpenCode 本地适配说明：在 OpenCode 中执行以下命令时，请显式把 Bash `workdir` 设为技能根目录 `~/.config/opencode/skills/web-access/`；如果从其他目录调用，请把 `./scripts/...` 改成实际绝对路径。上游原始 README 已保留为 `UPSTREAM-README.md` 供追溯。
...
获取页面基础信息（title、url、ready）。其中 `ready` 对应页面的 `document.readyState`。
